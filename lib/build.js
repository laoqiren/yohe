...
module.exports = function(dir,options){
    ...
    rd.eachFileFilterSync(sourceDir,(f,s)=>{
        ...
        post.url = `/${dateStr}/${utils.stripExtName(f.slice(sourceDir.length + 1))}/index.html`;
        list.push(post);
        ...
        if(postCategories in categories){
            ...
            categories[postCategories] = {
                name: postCategories,
                posts: [post],
                url: `/categories/${postCategories}/index.html`
            }
        }
    });
    ...
    // 渲染tags页
    renderTagsCates(dir,outputDir,'tags',tags);

    // 渲染categories页
    renderTagsCates(dir,outputDir,'categories',categories);
